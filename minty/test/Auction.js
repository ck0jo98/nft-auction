...
describe("Auction", function () {
  ...
  describe("Withdraw funds", () => {
    ...
    it("should not allow the highest bidder to withdraw their funds", async () => {
      await expect(
        await auction.connect(addr2).withdrawFunds()
      ).to.changeEtherBalances([addr2, auction], [0, 0]);
    });

    it("should not allow owner to withdraw until end() is called", async () => {
      await expect(
  ...
  describe("End", () => {
    it("shouldn't be callled until auction is complete", async () => {
    ...
    it("auction owner cannot withdraw funds once the auction is done", async () => {
      await expect(
        await auction.connect(owner).withdrawFunds()
